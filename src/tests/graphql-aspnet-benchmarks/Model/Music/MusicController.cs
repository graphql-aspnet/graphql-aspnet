...
namespace GraphQL.AspNet.Benchmarks.Model
{
    ...
    public class MusicController : GraphController
    {
        ...
        [QueryRoot("artist", typeof(Artist))]
        public async Task<IGraphActionResult> RetrieveSingleArtist([FromGraphQL("id")] int artistId)
        {
            var artists = await _musicService.RetrieveArtist(artistId);
        ...
        [BatchTypeExtension(typeof(Artist), "records", typeof(IEnumerable<Record>))]
