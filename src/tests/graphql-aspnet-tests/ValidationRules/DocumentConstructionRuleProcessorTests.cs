...
namespace GraphQL.AspNet.Tests.ValidationRules
{
    ...
    public class DocumentConstructionRuleProcessorTests
    {
        ...
        private ISyntaxTree CreateSyntaxTree(string text)
        {
            var parser = new GraphQLParser();
            return parser.ParseQueryDocument(text.AsMemory());
        }

        private IGraphQueryDocument CreateDocument(ISyntaxTree syntaxTree)
        {
            ...
            _documentProcessor.Execute(context);
            return doc;
        }
        ...
        {
            var tree = this.CreateSyntaxTree(text);
        ...
        {
            var tree = this.CreateSyntaxTree("fragment myData on Bagel @directive1 @directive2 {}");
