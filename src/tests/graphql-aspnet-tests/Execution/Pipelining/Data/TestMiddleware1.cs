...
namespace GraphQL.AspNet.Tests.Execution.Pipelining.Data
{
    ...
    /// </summary>
    public class TestMiddleware1 : IGraphFieldExecutionMiddleware
    {
        private readonly IMiddlewareTestService _testService;

        public TestMiddleware1(IMiddlewareTestService testService)
        {
        ...
        {
            _testService.BeforeNext(nameof(TestMiddleware1));
            await next(context, cancelToken);
            _testService.AfterNext(nameof(TestMiddleware1));
        }
    }
}