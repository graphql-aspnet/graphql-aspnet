...
namespace GraphQL.AspNet.Tests.Framework.PipelineContextBuilders
{
    ...
    public class DirectiveContextBuilder
    {
        ...
        private readonly Mock<IGraphDirectiveRequest> _mockRequest;
        private readonly Mock<IDirectiveInvocationContext> _mockInvocationContext;
            ...
            IGraphFieldResolverMetaData resolverMetadata)
        {
            ...
            _mockInvocationContext = new Mock<IDirectiveInvocationContext>();
            _mockInvocationContext.Setup(x => x.Directive).Returns(_directive);
            ...
            _mockInvocationContext.Setup(x => x.Location).Returns(() => _location);

            // fake the request for the directive
            // (normally generated by the primary query execution pipeline)
            var id = Guid.NewGuid();
            _mockRequest = new Mock<IGraphDirectiveRequest>();
            _mockRequest.Setup(x => x.Id).Returns(id);
            _mockRequest.Setup(x => x.Origin).Returns(SourceOrigin.None);
            _mockRequest.Setup(x => x.Directive).Returns(_directive);
            _mockRequest.Setup(x => x.DirectiveTarget).Returns(() => _directiveTarget);
            _mockRequest.Setup(x => x.InvocationContext).Returns(_mockInvocationContext.Object);
            _mockRequest.Setup(x => x.DirectivePhase).Returns(phase);
        ...
        public SchemaItemSecurityChallengeContext CreateSecurityContext()
        {
            ...
            var request = new SchemaItemSecurityRequest(this.DirectiveRequest.Object);
        ...
        public Mock<IGraphDirectiveRequest> DirectiveRequest => _mockRequest;
