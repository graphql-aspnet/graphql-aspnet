...
namespace GraphQL.AspNet.Schemas
{
    ...
    public static class GraphValidation
    {
        ...
        /// <summary>
        /// Determines if the provided type, if converted to a graph type, must be a
        /// leaf type and will never contain fields.
        /// </summary>
        /// <param name="typeToCheck">The type to check.</param>
        /// <returns><c>true</c> if the type must be converted to a leaf graph type, <c>false</c> otherwise.</returns>
        public static bool MustBeLeafType(Type typeToCheck)
        {
            Validation.ThrowIfNull(typeToCheck, nameof(typeToCheck));

            return typeToCheck.IsPrimitive || typeToCheck.IsEnum;
        }
